...
import { Trash2, User, Mail, Briefcase, Activity, AlertTriangle } from 'lucide-react'
import { isSameDay } from 'date-fns'

// Remove force-dynamic to allow partial optimization, though Auth usually requires dynamic.
...
export default async function EmployeesPage() {
    ...
    try {
        // Get Admin Company
        const { data: profile, error: profileError } = await supabase.from('profiles').select('Role:role, CompanyId:company_id').eq('id', user.id).single()

        if (profileError || !profile) {
            console.error("Profile Error:", profileError)
            return <div className="container">Erro ao carregar perfil. Tente recarregar.</div>
        }
        ...
        if (empError) {
            ...
            return <div className="container">Erro ao carregar lista de funcionários.</div>
        }
        ...
        return (
            <div className="container" style={{ textAlign: 'center', padding: '4rem' }}>
                ...
                <p>{e.message || 'Ocorreu um erro desconhecido.'}</p>
            </div>
        )
    }
}
